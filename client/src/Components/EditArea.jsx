...
export default function EditArea({
  ...
  function handleInputChange(value, paramIndex) {
    const id = Number(nodeId || 0);
    setParams((prev) =>
      prev.map((param, index) => (index === paramIndex ? value : param))
    );
    patchUserNodeByIndex(dispatch, id, {
      params:
        myNodes[id].params?.map((p, index) =>
          index === paramIndex ? value : p
        ) || [],
    });
  }
  ...
  function handleChangeMode(param, paramIndex) {
    ...
    patchUserNodeByIndex(dispatch, id, {
      params:
        myNodes[id].params?.map((p, index) =>
          index === paramIndex ? myNodes.length : p
        ) || [], // сетим myNodes.length, как id следующей ноды
    });
    setOperation();
    setParams([]);
          ...
          >
            {params.map((param, index) => (
              ...
              >
                ...
                {typeof myNodes[Number(nodeId)]?.params?.[index] ===
                "number" ? (
                  ...
                  <>
                    <Button
                      onClick={() => handleChangeMode(param, index)}
                      type="text"
                      size="small"
                    >
